...
with col1:
    ...
    if search_mode == t["by_ingredients"]:
        ...
        ingredients = st.text_area(t["ingredients"], value=selected_str, height=80, disabled=True)
        recipe_name = ""
    else:
...
"""
                ...
                if search_mode == t["by_image"]:
                    ...
                    if not image_bytes:
                        st.error("Uploaded image is empty. Please try another one.")
                        st.stop()
                    encoded_image = base64.b64encode(image_bytes).decode('utf-8')
                    img_type = getattr(uploaded_image, "type", "image/jpeg")
                    if not img_type or "image" not in img_type:
                        img_type = "image/jpeg"
...
{{
  ...
  "image_prompt": "[A highly detailed English prompt describing the visual appearance of the dish for an AI image generator]"
                ...
                # Use regex to extract the JSON object in case of preamble/postamble text
                import re
                ...
                try:
                    ...
                    st.error(f"Failed to parse recipe data. The AI returned invalid JSON. Please try again. (Error: {e})")
                    st.stop()
                ...
                with st.spinner("Finalizing your magical recipe (Generating Audio and PDF)... ✨"):
...
if st.session_state.current_recipe:
    ...
    with tab1:
        ...
        if gr.get("image_url"):
            st.image(gr["image_url"], caption=f"AI Generated Image for: {gr['recipe_title']}", use_container_width=True)
        ...
        if new_prompt:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    chat_llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0.7, google_api_key=gemini_api_key)
